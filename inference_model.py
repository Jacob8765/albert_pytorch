...
# def inference_model(df=ex_dataframe):
...
#     #raw_input = [InputExample(guid=f"test-{index}", text_a=instance, text_b=None, label="0") for index, instance in enumerate(sentences)]
#     results_dataframe = pd.DataFrame()

#     for row_index in range(df.shape[0]):
#         raw_input = [InputExample(guid=f"test-{row_index}-{i}", text_a=text, text_b=None, label="0") for i, text in enumerate(df.iloc[row_index, 3])]
...
#             # print(df)
#             # print(df.iloc[row_index, :].shape)
#             # print(logits_sigmoid.shape)
#             df.iloc[row_index, 2:] = logits_sigmoid.tolist()
#             # print(logits_softmax[:, 1])
